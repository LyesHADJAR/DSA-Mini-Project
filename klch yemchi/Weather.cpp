...
nlohmann::json Weather::weatherToJson() const {
    nlohmann::json json;
    json["CurrentCondition"] = CurrentCondition;
    json["MaxTemperature"] = MaxTemperature;
    json["MinTemperature"] = MinTemperature;
    json["SunnyHours"] = SunnyHours;
    return json;
}

Weather Weather::weatherFromJson(const nlohmann::json &jsonFile) {
    if (!jsonFile.is_object()) {
        throw std::invalid_argument("Invalid JSON format: Expected an object.");
    }
    Weather weather(
            jsonFile["CurrentCondition"].get<Weather_Conditions>(),
            jsonFile["MaxTemperature"].get<double>(),
            jsonFile["MinTemperature"].get<double>(),
            jsonFile["SunnyHours"].get<double>()
    );
    return weather;
}
